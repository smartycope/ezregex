...
class EZRegex(ABC):
    ...
    _exclusions = [
        '_escape_chars', '_repl_escape_chars', '_variables', '_allow_greedy',
        '_allow_possessive', 'invert', 'then'
    ]
    "Excluded methods"
    _added_vars = {}
    "A dict of {method name: dict} to manually add variables to methods"

    _canonical_parts: tuple[_str, ...] = ()
    "Canonical singleton names, captured before aliases are injected."

    _compound_parts: tuple[_str, ...] = ()
    "Canonical names generated by _add_compound_elements."

    _flag_map: dict[_str, _str] = {}
    _flag_docs_map: dict[_str, _str] = {}
    _flag_docs_link: _str = ''
...
'''
    ...
    def __init_subclass__(
        ...
        **kwargs
        ...
        compound_parts = cls._add_compound_elements()
        added |= compound_parts
        cls._compound_parts = tuple(sorted(compound_parts))
        cls._canonical_parts = tuple(sorted(added))
        ...
        cls._flag_map = flags.copy()
        cls._flag_docs_map = flags_docs_map.copy()
        cls._flag_docs_link = flags_docs_link
        ...
        for name, ps in psuedonyms.items():
            if hasattr(cls, name):
                value = getattr(cls, name)
                for p in ps:
                    # A pseudonym must never shadow a real canonical element.
                    if p in added:
                        continue
                    setattr(cls, p, value)
                    # also add camelCase versions of the psuedonyms
                    camel_name = _to_camel_case(p)
                    if camel_name not in added and not hasattr(cls, camel_name):
                        setattr(cls, camel_name, value)
    ...
    def parts(cls, include_psuedonyms=True, include_functions=True):#, include_replacement=True, _check_replacement=True):
        ...
        """
        # Once class construction has completed, use the exact names captured before
        # aliases were added. This avoids trying to reverse-engineer canonical names
        # from the much larger alias surface.
        if not include_psuedonyms and '_canonical_parts' in cls.__dict__:
            names = list(cls._canonical_parts)
            if include_functions:
                names.extend(
                    name for name in ('options', 'replace')
                    if hasattr(cls, name)
                )
            return names
    ...
    def __call__(self, *args:Unpack[List[EZRegexParam]], **kwargs:Unpack[Dict[_str, EZRegexParam]]):
        ...
        if not type(self).lazy_check_params:
            # Decorators preserve the public signature for introspection, but
            # parameter binding here must use the wrapper's chaining signature.
            sig = signature(self._func_list[-1], follow_wrapped=False)
